...
def test_get_model_and_optimizer_pspnet():
    ...
    print(isinstance(model, PSPNet))
    print(isinstance(optimizer, torch.optim.Optimizer))
    print(len(optimizer.param_groups) > 1)
    print(args.base_lr in param_learning_rates)
    print(not all([param_learning_rates == 1e-3]))
    print(type(model))
    print(PSPNet)
    print(type(PSPNet))
...
def test_pspnet_output_shapes() -> None:
    ...
    #print(isinstance(model.ppm, nn.Module))
    #print(isinstance(model.ppm, PPM))
